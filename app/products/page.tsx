  ...
  return (
    ...
    >
      <div className="relative w-full aspect-[4/5] bg-[#f9f8ff] rounded-3xl overflow-hidden mb-6 flex items-center justify-center shadow-sm border border-gray-50">
        ...
        <div className="absolute bottom-6 left-0 right-0 px-6 flex justify-center z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <button className="w-full bg-white/90 backdrop-blur-xl text-secondary py-3.5 rounded-full text-sm font-black shadow-xl flex justify-center items-center gap-2">
    ...
    <main className="min-h-screen bg-white dir-rtl">
      ...
      <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            <nav className="flex items-center gap-2 text-accent text-xs mb-4">
        ...
        <div className="flex flex-wrap gap-3 mb-12">
          {["همه", "کاپشن", "تی‌شرت", "هودی", "شلوار", "پیراهن"].map((cat) => (
            <button key={cat} className={`px-6 py-2.5 rounded-full text-sm font-bold border transition-all ${cat === "همه" ? "bg-secondary text-white border-secondary" : "bg-white text-secondary border-gray-100 hover:border-primary"}`}>
              {cat}
            </button>
          ))}
        </div>
